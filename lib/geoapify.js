...
export async function geocodeLocationWithGeoapify(locationName, userCoords = null) {
  ...
  try {
    ...
    // Dynamic Proximity Bias: Biases search to landmarks closest to user's live position (GPS or Coarse IP)
    let activeCoords = userCoords;
    if ((!activeCoords || !activeCoords.lat || !activeCoords.lng) && typeof window !== 'undefined') {
      try {
        const stored = localStorage.getItem('roomfind_user_gps');
        if (stored) {
          const parsed = JSON.parse(stored);
          if (parsed && parsed.lat && parsed.lng) {
            activeCoords = { lat: parsed.lat, lng: parsed.lng };
          }
        }
      } catch (_) {}
    }
    ...
    if (activeCoords && activeCoords.lat && activeCoords.lng) {
      url += `&bias=proximity:${activeCoords.lng},${activeCoords.lat}`;
    }
