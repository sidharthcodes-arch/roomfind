...
export default function ListingDetailPage({ params }) {
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/listings/${id}`, { headers })
        ...
        const mappedData = {
          ...data,
          id: data._id,
          users: data.user_id, // backend populates user_id
        }
        
        setListing(mappedData)
        setLikeCount(data._likeCount ?? 0)
        setLiked(data._liked ?? false)
        setSaved(data._saved ?? false)
      } catch (err) {
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/listings/${id}/like`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${user.accessToken}` }
      })
  ...
  const handleSave = async () => {
    ...
    setSaved(next)
    try {
      const res = await fetch(`http://localhost:5000/api/listings/${id}/save`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${user.accessToken}` }
      })
