...
function Toast({ message, type = "error", onDismiss }) {
  ...
  return (
    <div
      className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 ${bg} text-white text-[13px] px-4 py-3 rounded-xl shadow-lg max-w-sm w-full mx-4 flex items-center justify-between gap-3 animate-fade-in`}
    >
      <span>{message}</span>
      <button
        onClick={onDismiss}
  ...
  return (
    <div className="flex gap-3 items-start p-3 bg-white rounded-xl border border-black/[0.09]">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-slate-900 text-[14px] line-clamp-1">
          {listing.title}
        </p>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          >
            {isAvailable
              ? "✓ Available (Tap to Mark Taken)"
              : "✕ Taken (Tap to Mark Available)"}
          </button>
        </div>
      </div>
...
export default function ProfilePage() {
  ...
  const toggleListingStatus = async (id, currentStatus) => {
    ...
    if (error) {
      ...
      showToast(
        `Listing marked as ${nextStatus === "taken" ? "Taken" : "Available"}.`,
        "success",
      );
    }
  };
        ...
        {profile?.role === "landlord" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Link
                href="/create-listing"
                className="flex items-center gap-1 text-brand text-[13px] font-medium"
              >
            ...
            {listingsLoading ? (
              <div className="space-y-2 animate-pulse">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="h-24 w-full bg-slate-200 rounded-xl"
                  />
                ))}
              </div>
            ) : listings.length === 0 ? (
              <div className="bg-white rounded-2xl p-6 border border-dashed border-black/[0.09] text-center">
                ...
                <Link
                  href="/create-listing"
                  className="mt-2 inline-block text-brand text-[13px] font-medium underline underline-offset-2"
                >
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  Select your country code and enter your WhatsApp contact
                  number.
                </p>
              </div>
