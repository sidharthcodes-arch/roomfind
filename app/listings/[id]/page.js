...
function Toast({ message, type = "success", onDismiss }) {
  ...
  return (
    <div
      className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 ${bg} text-white text-[13px] font-medium px-4 py-3 rounded-xl shadow-lg max-w-sm w-full mx-4 flex items-center justify-between gap-3 animate-fade-in`}
    >
      <span>{message}</span>
      <button
        onClick={onDismiss}
...
export default function ListingDetailPage({ params }) {
    ...
    const fetchComments = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select(
            "id, comment_text, created_at, user_id, users(full_name, profile_photo)",
          )
          .eq("listing_id", id)
  ...
  const handleCopyAddress = () => {
    const fullAddr = [listing.address, listing.area, listing.city]
      .filter(Boolean)
      .join(", ");
    if (!fullAddr) return;
    ...
    <div className="min-h-screen bg-[#ececea] pb-28 max-w-md mx-auto relative border-x border-black/[0.05]">
        ...
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11.5px] text-slate-400 font-normal truncate mt-0.5">
              {[listing.city, listing.state || "West Bengal"]
                .filter(Boolean)
                .join(", ")}
            </p>
          </div>
      ...
      <div className="p-3 space-y-3.5">
        ...
        <div className="bg-white rounded-2xl border border-black/[0.09] shadow-sm overflow-hidden space-y-0">
          ...
          {photos.length > 0 ? (
            <div
              className="relative bg-slate-900 aspect-[4/3] overflow-hidden group cursor-pointer"
            ...
            >
              ...
              <div className="absolute top-2.5 left-2.5 flex items-center gap-1.5 z-10">
                <span
                  className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full shadow-xs flex items-center gap-1.5 ${
                    isTaken
                ...
                >
                  <svg
                    className="w-3 h-3 fill-current text-white shrink-0"
                    width="12"
                    height="12"
                    viewBox="0 0 20 20"
                  >
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <p className="text-[13.5px] text-slate-600 font-medium mt-1 flex items-start gap-1">
                  <svg
                    className="w-4 h-4 fill-current text-[#00a884] shrink-0 mt-0.5"
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                  >
              ...
              <div className="grid grid-cols-3 gap-2.5">
                <div className="bg-emerald-50/50 rounded-2xl p-3 border border-emerald-100/60 text-center flex flex-col items-center">
                  <div className="w-8 h-8 rounded-full bg-emerald-100/80 text-emerald-700 flex items-center justify-center mb-1">
                    <svg
                      className="w-4 h-4 fill-current text-emerald-700"
                      width="16"
                      height="16"
                      viewBox="0 0 20 20"
                    >
            ...
            <div className="pt-2.5 border-t border-slate-100 space-y-3">
              <div className="flex items-center justify-between text-slate-600 text-sm font-semibold">
                ...
                >
                  <Heart
                    className={`w-4.5 h-4.5 ${liked ? "fill-coral text-coral" : "text-slate-400"}`}
                  />
                  <span className={liked ? "text-coral font-bold" : ""}>
                    {likeCount}
                  </span>
                </button>
                ...
                >
                  <svg
                    className="w-4.5 h-4.5 fill-current text-slate-500 shrink-0"
                    width="18"
                    height="18"
                    viewBox="0 0 24 24"
                  >
              ...
              {showComments && (
                <div className="pt-3.5 border-t border-slate-100 space-y-3.5 animate-fade-in">
                  ...
                  <div className="space-y-3 max-h-64 overflow-y-auto pr-1 scrollbar-thin">
                    {commentsLoading ? (
                      ...
                      <div className="py-4 text-center text-xs text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                        No comments yet. Be the first to ask the owner a
                        question!
                      </div>
                    ) : (
                      comments.map((comment) => {
                        ...
                        const isOwner =
                          (listing?.user_id &&
                            comment.user_id === listing.user_id) ||
                          (listing?.users?.id &&
                            comment.user_id === listing.users.id);
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="bg-slate-50 border border-slate-100 rounded-2xl px-3.5 py-2.5 flex-1 min-w-0">
                              ...
                              <p className="text-[13.5px] text-slate-700 mt-1 leading-snug break-words">
                                {commentText}
                              </p>
                            </div>
                          </div>
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      className="w-9 h-9 rounded-full bg-[#00a884] hover:bg-[#008f6f] text-white flex items-center justify-center shrink-0 shadow-xs transition-transform active:scale-95"
        ...
        {/* UNIFIED LOCATION & NAVIGATION CARD */}
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
          {/* Unified Location & Address Header Block */}
          <div className="p-3.5 px-4 bg-slate-50/70 border-b border-slate-100 space-y-2.5">
            {/* Top row: Header & Distance Badge */}
            <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-200/70">
              <div className="flex items-center gap-2 min-w-0">
                <svg
                  className="w-6 h-6 fill-current text-emerald-600 shrink-0"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                >
              ...
              <span className="text-xs font-semibold text-emerald-700 bg-white border border-emerald-200/80 px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-2xs shrink-0">
                <Route className="w-3.5 h-3.5 text-coral shrink-0" />
                <span>
            ...
            <div className="flex items-start justify-between gap-3 pt-1">
              ...
              <button
                ...
                className="shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-white border border-slate-200/80 hover:bg-slate-100 text-slate-700 text-xs font-bold transition-colors shadow-2xs"
              >
